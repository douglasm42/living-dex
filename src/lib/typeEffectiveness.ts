...
export const TYPE_CHART: Record<PokemonType, Record<PokemonType, DamageMultiplier>> = {
  normal: {
  ...
  ghost: {
    ...
    fighting: 0, poison: 1, ground: 1, flying: 1, psychic: 2, bug: 1,
  ...
  fairy: {
    ...
    fighting: 2, poison: 0.5, ground: 1, flying: 1, psychic: 2, bug: 1,
...
export function getDefensiveEffectiveness(types: string[]): TypeEffectiveness[] {
  ...
  for (const attackType of allTypes) {
    ...
    for (const defenderType of types) {
      if (defenderType in TYPE_CHART[attackType]) {
        multiplier *= TYPE_CHART[attackType][defenderType as PokemonType]
      }
    }
    effectiveness.push({ type: attackType, multiplier })
  }

  return effectiveness
}

export function getWeaknesses(types: string[]): TypeEffectiveness[] {
  return getDefensiveEffectiveness(types)
    .filter(e => e.multiplier > 1)
    .sort((a, b) => b.multiplier - a.multiplier)
}

export function getResistances(types: string[]): TypeEffectiveness[] {
  return getDefensiveEffectiveness(types)
    .filter(e => e.multiplier < 1 && e.multiplier > 0)
    .sort((a, b) => a.multiplier - b.multiplier)
}

export function getImmunities(types: string[]): TypeEffectiveness[] {
  return getDefensiveEffectiveness(types)
